...
export default function InvoiceVouchersPage() {
  ...
  const saveLineForm = async (e) => {
    ...
    try {
      ...
      // Ensure catalog item exists if they typed a name or added an image, but haven't linked
      if (!finalItemId && (lineForm.itemName.trim() || lineForm.itemNo.trim() || imageBase64)) {
        const newItemName = lineForm.itemName.trim() || lineForm.itemNo.trim() || "منتج جديد";
      ...
      // refresh catalog so new items/images are reflected in dropdown
              ...
              lines.map((r) => (
                ...
                >
                  ...
                  <td style={{ textAlign: "center" }}>
                    {r.item?.imageUrl ? (
                       <img src={r.item.imageUrl} alt={r.itemName || r.itemNo} style={{ maxWidth: 40, maxHeight: 40, borderRadius: 4, border: "1px solid #ccc" }} />
                    ) : "—"}
                  </td>
